 ...
 * client-side element that can force the enclosing form to submit. The submit
 * responsible for the form submission will post a notification that allows the
 * application to know that it was the responsible entity. The notification is
 * named {@linkplain EventConstants#SELECTED selected}, by default, and has no
 * context.
 *
 * @tapestrydoc
 */
...
@org.apache.tapestry5.annotations.Import(module = "t5/core/forms")
public class Button implements ClientElement {
	/**
	 * If true (the default), then any notification sent by the component will
	 * be deferred until the end of the form submission (this is usually
	 * desirable). In general, this can be left as the default except when the
	 * Submit component is rendering inside a {@link Loop}, in which case defer
	 * should be bound to false (otherwise, the event context will always be the
	 ...
	 * cause of the form submission. The default is
	 * {@link EventConstants#SELECTED}.
	 */
	...
	/**
	 * The list of values that will be made available to event handler method of
	...
	/**
	 * Defines the mode, or client-side behavior, for the submit. The default is
	 * {@link SubmitMode#NORMAL}; clicking the button submits the form with
	 * validation. {@link SubmitMode#CANCEL} indicates the form should be
	 * submitted as a cancel, with no client-side validation.
	 * {@link SubmitMode#UNCONDITIONAL} bypasses client-side validation, but
	 * does not indicate that the form was cancelled.
	 *
	...
	@Parameter(value = "submit", defaultPrefix = BindingConstants.LITERAL)
	private String type;

	private static class ProcessSubmission implements ComponentAction<Button> {
		...
		@Override
		public void execute(Button component) {
	...
	private boolean selected(String clientId, String elementName) {
		...
		// Case #2: No JavaScript, look for normal semantic (non-null value for
		// the element's name).
