...
public class ToolsService {
	...
	public String getTime(int time) {
		...
		DateTimeFormatter fmt = new DateTimeFormatterBuilder().appendPattern("d hh:mm").toFormatter();
		LocalDateTime time1 = LocalDateTime.ofEpochSecond(time, 0, ZoneOffset.ofHours(0));
		return time1.format(fmt);
		// return fmt.format(new Date(time * 1000).toInstant());
	}
